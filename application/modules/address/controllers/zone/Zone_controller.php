...
class Zone_controller extends MX_Controller {
    
    public function __construct() {
        parent::__construct ();
		$this->load->model('address/zone/Zone_model');
    ...
    public function FSaCZNEEditEvent(){
		...
		$this->db->trans_begin();
		$aCheckLangAdded = $this->Zone_model->FSaMZNECheckLangAdded($this->input->post('oetZneCode'),$this->session->userdata("tLangEdit"));
		$nCheckLangAdded = $aCheckLangAdded[0]->counts;

		if($nCheckLangAdded > 0){
			$aResAdd3 = $this->Zone_model->FSaMZNEUpdateZneNameAndFTZneChainNameMaster($aDataMaster);
		}else{
			$aResAddLang = $this->Zone_model->FSnMZNEAddLang($aDataMaster); /* Add ภาษา ที่ยังไม่มีในตาราง L */
		}

		$aResUpdateArea = $this->Zone_model->FSaMZNEUpdateArea($aDataArea);
    ...
    public function FSaCZNEAddEvent(){
    	...
    	if($nSelectRoot == 'on'){
    		...
    		$aDataZoneSelect = $this->Zone_model->FSaMZNEGetZneLevelANDZneChain($tZneCodeSelect);
    		$nZneLevelSelect = $aDataZoneSelect[0]->FNZneLevel;
    		$tZneChainSelect = $aDataZoneSelect[0]->FTZneChain;
    		
    		$aDataZneChainNameSelect = $this->Zone_model->FSaMZNEGetZneSelectZneChainName($tZneCodeSelect);
		...
		$oCountDup  = $this->Zone_model->FSnMZENCheckDuplicate($aDataMaster['FTZneCode']);
		$nStaDup    = $oCountDup[0]->counts;
		if($nStaDup == 0){
			$this->db->trans_begin();
				$aResAdd     = $this->Zone_model->FSaMZNEAdd($aDataMaster);
				$aResAddLang = $this->Zone_model->FSnMZNEAddLang($aDataMaster); /* Add ภาษา ที่ยังไม่มีในตาราง L */
			if($this->db->trans_status() === FALSE){
				$this->db->trans_rollback();
				$aReturnData    = array(
...
//     		$this->load->view('address/branch/wBranchList',$aHTML);
    ...
    public function FSvCZNEDataList(){
		...
		$aResList   	= $this->Zone_model->FSnMZNEList($aData);
		$aAlwEventZone 	= FCNaHCheckAlwFunc('zone/0/0'); 
        $aGenTable  = array(
            'aDataList' 	=> $aResList,
            'nPage'     	=> $nPage,
  ...
  //Functionality : Event Zone Delete
    ...
    public function FSaCZNEDeleteEvent(){
		...
		$aResDel 	= $this->Zone_model->FSnMZNEDel($aDataMaster);
		$nNumRowZneLoc  = $this->Zone_model->FSnMLOCGetAllNumRow();
		$aReturn    = array(
    ...
    public function FSvCZNEAddRefer(){
		...
		try {
			...
			if(isset($tZneCode) && !empty($tZneCode)){
				$aResAdd    = $this->Zone_model->FSaMZNEAddUpdateMaster($aDataMaster);
			}
			if($this->db->trans_status() === FALSE){
				$this->db->trans_rollback();
				$aReturnData    = array(
    ...
    public function FSvCZNEObjDataList(){
		...
		$aResList   	= $this->Zone_model->FSnMZNEObjList($aData);
    ...
    public function FSvCZNEEditRefer(){
		...
		try {
			...
			$this->db->trans_begin();
			$aResAdd    = $this->Zone_model->FSaMZNEReferUpdateMaster($aDataMaster);
			if($this->db->trans_status() === FALSE){
				$this->db->trans_rollback();
				$aReturnData    = array(
					'nStaEvent'     => '900',
