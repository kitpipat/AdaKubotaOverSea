...
class Banknote_controller extends MX_Controller {

    public function __construct(){
        parent::__construct ();
        $this->load->model('payment/banknote/Banknote_model');
    }
    ...
    public function FSvCBNTEditPage(){
        ...
        $aResList   = $this->Banknote_model->FSaMBNTGetDataByID($aData);
    ...
    public function FSoCBNTAddEvent(){ 
        ...
        if($oCountDup !== FALSE && $nStaDup == 0){
            $this->db->trans_begin();
            $this->Banknote_model->FSaMBNTAddUpdateMaster($aDataBnt);
            $this->Banknote_model->FSaMBNTAddUpdateLang($aDataBnt);
    ...
    public function FSoCBNTEditEvent(){ 
        ...
        $aStaBntLang    = $this->Banknote_model->FSaMBNTAddUpdateLang($aDataBnt);
        if($this->db->trans_status() === false){
            $this->db->trans_rollback();
            $aReturn = array(
                'nStaEvent' => '900',
    ...
    public function FSoCBNTDeleteEvent(){
        ...
        $aResDel    = $this->Banknote_model->FSaMBNTDelAll($aDataMaster);
        $nNumRowBntLoc = $this->Banknote_model->FSnMLOCGetAllNumRow();
    ...
    public function FStCBanknoteUniqueValidate()
    {
        ...
        if ($this->input->is_ajax_request()) { // Request check
            ...
            $bIsDocNoDup = $this->Banknote_model->FSbMCheckDuplicate($tBanknoteCode);
