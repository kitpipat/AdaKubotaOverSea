...
class Coupontype_controller extends MX_Controller {

	public function __construct() {
            parent::__construct ();
            $this->load->model('Coupontype/Coupontype_model');
    }
    ...
    public function FSaCCPTAddEvent(){
        ...
        try{
            ...
            $oCountDup  = $this->Coupontype_model->FSnMCPTCheckDuplicate($aDataMaster['FTCptCode']);
            $nStaDup    = $oCountDup[0]->counts;
    
            if($nStaDup == 0){
                $this->db->trans_begin();
                $aStaEventMaster  = $this->Coupontype_model->FSaMCPTAddUpdateMaster($aDataMaster);
                $aStaEventLang    = $this->Coupontype_model->FSaMCPTAddUpdateLang($aDataMaster);
    ...
    public function FSaCCPTDeleteEvent(){
        ...
        $aResDel   = $this->Coupontype_model->FSnMCPTDel($aDataMaster);
        $nNumRowCPTType = $this->Coupontype_model->FSnMLOCGetAllNumRow();
    ...
    public function FSvCCPTEditPage(){
        ...
        $aResult       = $this->Coupontype_model->FSaMCPTSearchByID($aData);
    ...
    public function FSxCCPTDataTable(){
        ...
        $aResList   = $this->Coupontype_model->FSaMCPTList($aData);
        $aGenTable  = array(
