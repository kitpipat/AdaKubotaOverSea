...
// ตัวแทนขาย
class Agency_controller extends MX_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model('ticket/agency/Agency_model');
    ...
    public function FSxCANGDataTable(){
        ...
        $aResList   = $this->Agency_model->FSaMAGNList($aData);
        $aGenTable  = array(
    ...
    public function FSvCAGNEditPage(){
        ...
        $aResult        = $this->Agency_model->FSaMAGNSearchByID($aData);
    ...
    public function FSaCAGNAddEvent(){
        ...
        try{
            ...
            $oCountDup  = $this->Agency_model->FSnMAGNCheckDuplicate ($aDataMaster['FTAgnCode']);
            $nStaDup    = $oCountDup[0]->counts;
            if($nStaDup == 0){
                $this->db->trans_begin();
                $aStaEventMaster  = $this->Agency_model->FSaMAGNAddUpdateMaster($aDataMaster);
                $aStaEventLang    = $this->Agency_model->FSaMAGNAddUpdateLang($aDataMaster);
    ...
    public function FSaCAGNDeleteEvent(){
        ...
        $aResDel        = $this->Agency_model->FSnMAGNDel($aDataMaster);
        $nNumRowAgn     = $this->Agency_model->FSnMAGNGetAllNumRow();
        $aDeleteImage = array(
                'tModuleName'  => 'ticket',
                'tImgFolder'   => 'ticketagency',
                'tImgRefID'    => $tIDCode ,
