...
class Interfaceimport_controller extends MX_Controller
{
    ...
    public function index($nBrowseType, $tBrowseOption)
    {
        ...
        $aData['aDataMasterImport'] = $this->Interfaceimport_model->FSaMINMGetHD($tLangEdit);


        // $this->Interfaceimport_model->FSaMINMGetDataConfig($tLangEdit);
    ...
    public function FSxCINMCallRabitMQ()
    {
        ...
        if ($tTypeEvent == 'getpassword') {
            $aResult = $this->Interfaceimport_model->FSaMINMGetDataConfig($tLangEdit);
            if($aResult['rtCode'] != 1){
                $aConnect = array(
                    'tHost'      => '',
                    'tPort'      => '',
    ...
    function FCNxCallRabbitMQMaster($paParams, $pbStaUse = true, $ptPasswordMQ)
    {
        ...
        $tLangEdit  = $this->session->userdata("tLangEdit");
        $aVal       = $this->Interfaceimport_model->FSaMINMGetDataConfig($tLangEdit);
