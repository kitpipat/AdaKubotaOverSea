...
class Promotionstep4zonecondition_model extends CI_Model
{
    /**
    ...
    public function FSaMGetPdtPmtHDChnInTmp($paParams = [])
    {
        ...
        $tSQL = "
            SELECT c.* FROM(
                SELECT  ROW_NUMBER() OVER(ORDER BY FTZneName ASC) AS FNRowID,* FROM
                    (SELECT DISTINCT
                        TMP.FTBchCode,
                        TMP.FTZneCode,
                        TMP.FTPmhDocNo,
    ...
    public function FSaMPdtPmtHDChnToTemp($paParams = [])
    {
        ...
        $this->db->set('FTZneCode', $paParams['tZoneCode']);
        $this->db->set('FTZneChain', $paParams['tZoneChain']);
        $this->db->set('FTZneName', $paParams['tZoneName']);
        ...
        $this->db->insert('TCNTPdtPmtHDZne_Tmp');

        $aStatus = [
            'rtCode' => '905',
            'rtDesc' => 'Insert TCNTPdtPmtHDZone_Tmp Fail.',
        ];

        if ($this->db->affected_rows() > 0) {
            $aStatus['rtCode'] = '1';
            $aStatus['rtDesc'] = 'Insert TCNTPdtPmtHDZone_Tmp Success';
        }
        return $aStatus;
    }
    ...
    public function FSbUpdateChnInTmpByKey($paParams = [])
    {
        ...
        $this->db->where('FTZneChain', $paParams['tZneCode']);
        $this->db->where('FTPmhDocNo', $paParams['tDocNo']);
        $this->db->where('FTZneCode', $paParams['tChnCode']);
    ...
    public function FSbDeletePdtPmtHDChnInTmpByKey($paParams = [])
    {
        $this->db->where('FTPmhDocNo', $paParams['tDocNo']);
        $this->db->where('FTBchCode', $paParams['tBchCode']);
        $this->db->where('FTZneChain', $paParams['tZoneChainCode']);
        $this->db->where('FTSessionID', $paParams['tUserSessionID']);
