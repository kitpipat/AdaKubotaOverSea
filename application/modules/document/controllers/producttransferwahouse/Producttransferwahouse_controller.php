...
class Producttransferwahouse_controller extends MX_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('document/producttransferwahouse/Producttransferwahouse_model');
    }
    ...
    public function FSvCTFWGetShipAdd()
    {
        ...
        $aDataShipAdd = $this->Producttransferwahouse_model->FSaMTFWGetAddress($tBchCode, $tXthShipAdd, $nLangEdit);
    ...
    public function FSaCTFWEditEvent()
    {
        try {
            ...
            $this->db->trans_begin();
            $aStaSdtOrdHD               = $this->Producttransferwahouse_model->FSaMTFWAddUpdateHD($aDataMaster);
            $aStaEventOrdHDRef          = $this->Producttransferwahouse_model->FSaMTFWAddUpdateHDRef($aDataHDSpl);
            //$aStaEventDocNoInDocTemp    = $this->Producttransferwahouse_model->FSaMTFWAddUpdateDocNoInDocTemp($aDataWhere);    //Update DocNo ในตาราง Doctemp
    ...
    public function FSaMTFWAddTmpToDT($ptXthDocNo = '')
    {
        ...
        $aResInsDT      =  $this->Producttransferwahouse_model->FSaMTFWInsertTmpToDT($aDataWhere);
    }
    ...
    public function FSaMTFWSumDTIntoHD($ptXthDocNo)
    {
        ...
        $aDataDT    =   $this->Producttransferwahouse_model->FSaMTFWGetDT($aData); //*ลบ Data เก่าออก*/
        ...
        $aDataDT    =   $this->Producttransferwahouse_model->FSaMTFWUpdateHDFCXthTotal($aDataUpdHD, $tXthDocNo); //*ลบ Data เก่าออก*/
    }
    ...
    public function FSvCTFWEditPdtIntoTableDT()
    {
        ...
        //Edit Inline
        $aResUpdDTTmpInline     = $this->Producttransferwahouse_model->FSnMTFWUpdateInlineDTTemp($aDataUpdateDT, $aDataWhere);
    }
    ...
    public function FSnTFWUpdateHD()
    {
        ...
        //Get ค่า VATRate
        $cVatRate = $this->Producttransferwahouse_model->FScMTFWGetVatRateFromDoc($tXthDocNo);

        //Get ค่า VAT InOrEx
        $tXthVATInOrEx = $this->Producttransferwahouse_model->FCNxTFWGetvatInOrEx($tXthDocNo);

        //get จาก DT
        //ยอดรวมก่อนลด
        $cXthTotal          = $this->Producttransferwahouse_model->FSaMTFWGetHDFCXthTotal($tXthDocNo);
        //ยอดรวมมีภาษีห้ามลด
        $cXthVatNoDisChg    = $this->Producttransferwahouse_model->FSaMTFWGetHDFCXthVatNoDisChg($tXthDocNo);
        // echo "ยอดรวมมีภาษีห้ามลด :".$cXthVatNoDisChg."<br>";
        //ยอดรวมไม่มีภาษีห้ามลด
        $cXthNoVatNoDisChg  = $this->Producttransferwahouse_model->FSaMTFWGetHDFCXthNoVatNoDisChg($tXthDocNo);
        // echo "ยอดรวมไม่มีภาษีห้ามลด :".$cXthNoVatNoDisChg."<br>";
        //ยอดมีภาษีลดได้
        $cXthVatDisChgAvi   = $this->Producttransferwahouse_model->FSaMTFWGetHDFCXthVatDisChgAvi($tXthDocNo);
        // echo "ยอดมีภาษีลดได้ :".$cXthVatDisChgAvi."<br>";
        //ยอดไม่มีภาษีลดได้
        $cXthNoVatDisChgAvi = $this->Producttransferwahouse_model->FSaMTFWGetHDFCXthNoVatDisChgAvi($tXthDocNo);
        // echo "ยอดไม่มีภาษีลดได้ :".$cXthNoVatDisChgAvi."<br>";

        //ข้อมูลการ ลด ชาร์จ
        $aXthDisChgTxt = $this->Producttransferwahouse_model->FSaMTFWGetHDFTXthDisChgTxt($tXthDocNo);
        $tDisChgTxt = '';
        if ($aXthDisChgTxt != 0) {
        ...
        //get จาก HDDis
        //มูลค่ารวมส่วนลด
        $cXthDis            = $this->Producttransferwahouse_model->FSaMTFWGetHDFCXthDis($tXthDocNo);
        //มูลค่ารวมส่วนชาร์จ
        $cXthChg            = $this->Producttransferwahouse_model->FSaMTFWGetHDFCXthChg($tXthDocNo);

        //ยอดรวมมีภาษีหลังลด FCXthVatDisChgAvi-SUM(HDis.FCXddDisVat-HDis.FCXddChgVat)
        //Get SUM(HDis.FCXddDisVat-HDis.FCXddChgVat)
        $cXthDisRes    = $this->Producttransferwahouse_model->FSaMTFWGetSUMFCXddDisVatMinusFCXddChgVat($tXthDocNo);
        $cXthVatAfDisChg    = $cXthVatDisChgAvi - $cXthDisRes;
        // echo "ยอดรวมมีภาษีหลังลด :".$cXthVatAfDisChg."<br>";

        //ยอดรวมไม่มีภาษีหลังลด
        $cXthNoVatAfDisChg  = $this->Producttransferwahouse_model->FSaMTFWGetHDFCXthNoVatAfDisChg($tXthDocNo);
        // echo $cXthNoVatAfDisChg."<br>";

        //ยอดมัดจำ
        $cFCXthRefAEAmt  = $this->Producttransferwahouse_model->FSaMTFWGetFCXthRefAEAmt($tXthDocNo);
        ...
        if ($oetXthWpTax != '') {
            ...
            $cXthWpTax = $this->Producttransferwahouse_model->FSaMTFWGetHDFCXthWpTax($tXthDocNo);
        }
        ...
        $aStaUpdOrdHD = $this->Producttransferwahouse_model->FSaMTFWUpdateOrdHD($Data, $DataWhere); /*ลงตาราง TAPTOrdHD */
    }
    ...
    public function FSvCTFWApprove()
    {
        ...
        $aStaApv = $this->Producttransferwahouse_model->FSvMTFWApprove($aDataUpdate);
    ...
    public function FSvCTFWCancel()
    {
        ...
        $aStaApv = $this->Producttransferwahouse_model->FSvMTFWCancel($aDataUpdate);
    ...
    public function FSaMTFWAddDT()
    {
        ...
        $aStaEventDelDT =  $this->Producttransferwahouse_model->FSnMPMTDelPcoDT($this->input->post('oetSearchAll')); //*ลบ Data เก่าออก*/
        ...
        if ($nNum != 0) {
            foreach ($aDataFile['DTData'] as $key => $value) {
                ...
                $aStaEventOrdHDSpl  = $this->Producttransferwahouse_model->FSaMTFWAddUpdateOrdDT($value); /*ลงตาราง TAPTOrdDT */
    ...
    public function FSaMTFWAddDTDis()
    {
        ...
        $aStaEventDelDTDis =  $this->Producttransferwahouse_model->FSnMPMTDelPcoDTDis($tXthDocNo); //*ลบ Data เก่าออก*/
        ...
        if ($nNum != 0) {
            ...
            foreach ($aDataFile['DTData'] as $key => $valueDT) {
                ...
                foreach ($valueDT['DTDiscount'] as $keyDis => $valueDTDis) {
                    ...
                    $aStaEventOrdDTDis  = $this->Producttransferwahouse_model->FSaMTFWAddUpdateOrdDTDis($valueDTDis); /*ลงตาราง TAPTOrdDTDis */
                ...
                $aStaUpdDT  = $this->Producttransferwahouse_model->FSaMTFWUpdateOrdDT($aDTData, $aDTDataWhere); /*Update TAPTOrdDT ใหม่ */
    ...
    public function FSaMTFWAddHDDis()
    {
        ...
        $aStaEventDelHDDis =  $this->Producttransferwahouse_model->FSnMPMTDelPcoHDDis($tXthDocNo); //* ลบ Data เก่าออก */
        ...
        if ($nNum > 0) {
            ...
            foreach ($aDataFile['HDData'] as $key => $value) {
                ...
                $aStaEventOrdHDDis  = $this->Producttransferwahouse_model->FSaMTFWAddUpdateHDDis($value); /*ลงตาราง TAPTOrdDT*/
    ...
    public function FSvCTFWAddPdtIntoTableDT()
    {
        ...
        $nCounts    = $this->Producttransferwahouse_model->FSaMTFWGetCountDTTemp($aDataWhere);
        ...
        for ($nI = 0; $nI < count($aPdtData); $nI++) {
            ...
            $aDataPdtMaster     =   $this->Producttransferwahouse_model->FSaMTFWGetDataPdt($aDataPdtWhere); // Data Master Pdt





            $nStaInsPdtToTmp    =   $this->Producttransferwahouse_model->FSaMTFWInsertPDTToTemp($aDataPdtMaster, $aDataPdtWhere);
        }
    ...
    */
    public function FSnCTFWCalulateDTTemp($ptXthDocNo, $ptXthVATInOrEx)
    {
        ...
        //Get DT Tmp
        $aDataDTTmp = $this->Producttransferwahouse_model->FSaMTFWGetDTTemp($aDataWhere);
        ...
        if ($aDataDTTmp['rtCode'] == 1) {
            ...
            $aResUpd = $this->Producttransferwahouse_model->FSnMWTOUpdateDTTemp($aDataDTTmp, $aDataWhere);
        }
    }
    ...
    public function FSaCTFWDeleteEvent()
    {
        ...
        $aResDel    = $this->Producttransferwahouse_model->FSnMTFWDel($aDataMaster);
        $aReturn    = array(
    ...
    public function FSvCTFWRemovePdtInDTTmp()
    {
        ...
        $aResDel = $this->Producttransferwahouse_model->FSnMTFWDelDTTmp($aDataWhere);
    }
    ...
    {
        $this->Producttransferwahouse_model->FSxMClearPdtInTmp();
        //Get Option Show Decimal
        $nOptDecimalShow = FCNxHGetOptionDecimalShow();
        ...
        $aDataUserGroup = $this->Producttransferwahouse_model->FStTFWGetShpCodeForUsrLogin($aDataShp); // Get ข้อมูลสาขา และร้านค้าของ User ที่ login
    ...
    public function FSbCheckHaveProductForTransfer()
    {
        ...
        $nNumPdt = $this->Producttransferwahouse_model->FSnMTFWCheckPdtTempForTransfer($tDocNo);
        if ($nNumPdt > 0) {
    ...
    public function FSbCheckHaveProductInDT()
    {
        $tDocNo = $this->input->post("tDocNo");
        $tBchCode = $this->input->post("tBchCode");
        $nNumPdt = $this->Producttransferwahouse_model->FSnMTFWCheckHaveProductInDT($tDocNo, $tBchCode);
        if ($nNumPdt > 0) {
            echo json_encode(true);
        } else {
            echo json_encode(false);
    ...
    {
        // $tXthDocNo = $this->input->post("ptXthDocNo");
        // $aGetInforTwxHD = $this->Producttransferwahouse_model->FSxMTWFGeTInforTwxHD($tXthDocNo);
        // $aGetInforTwxHDRef = $this->Producttransferwahouse_model->FSxMTWFGeTInforTwxHDRef($tXthDocNo);
        // $aGetInforTwxDT = $this->Producttransferwahouse_model->FSxMTWFGeTInforTwxDT($tXthDocNo);
        ...
        //Get Data
        $aResult        = $this->Producttransferwahouse_model->FSaMTFWGetHD($aDataWhere);       // Data TCNTPdtTwxHD
        $aDataHDRef     = $this->Producttransferwahouse_model->FSaMTFWGetHDRef($aDataWhere);    // Data TCNTPdtTwxHDRef
        $aDataDT        = $this->Producttransferwahouse_model->FSaMTFWGetDT($aDataWhere);       // Data TCNTPdtTwxDT
        $aStaIns        = $this->Producttransferwahouse_model->FSaMTFWInsertDTToTemp($aDataDT, $aDataWhere);    // Insert Data DocTemp
    ...
    public function FSxCTFWDataTable()
    {
        ...
        $aResList   = $this->Producttransferwahouse_model->FSaMTFWList($aData);
        $aGenTable  = array(
            'aAlwEvent'     => $aAlwEvent,
            'aDataList'     => $aResList,
            'nPage'         => $nPage,
    ...
    public function FSvCTFWPdtAdvTblLoadData()
    {
        ...
        $aDataDT        = $this->Producttransferwahouse_model->FSaMTFWGetDTTempListPage($aDataWhere);
        $aDataDTSum     = $this->Producttransferwahouse_model->FSaMTFWSumDTTemp($aDataWhere);
    ...
    */
    public function FSvCTFWVatLoadData()
    {
        ...
        $aDataVatDT     = $this->Producttransferwahouse_model->FSaMTFWGetVatDTTemp($aDataWhere);
    ...
    */
    ...
    public function FSvCTFWAdvTblShowColList()
    {
        ...
        $aAvailableColumn           = $this->Producttransferwahouse_model->FSoMTFWlableColumn('TCNTPdtTwxDT');
    ...
    public function FSvCTFWPdtMultiDeleteEvent()
    {
        ...
        if ($nCount > 1) {
            ...
            for ($i = 0; $i < $nCount; $i++) {
                ...
                $aResDel = $this->Producttransferwahouse_model->FSaMTFWPdtTmpMultiDel($aDataMaster);
            }
        } else {
    ...
    public function FSxCTFXClearDocTemForChngCdt()
    {
        ...
        $this->Producttransferwahouse_model->FSxMTFXClearDocTemForChngCdt(array(
    ...
    public function FSxCTWXCheckViaCodeForApv()
    {
        ...
        $aResult = $this->Producttransferwahouse_model->FSxMTWXCheckViaCodeForApv($tDocNo);
        if ($aResult) {
