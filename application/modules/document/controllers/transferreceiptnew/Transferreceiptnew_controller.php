...
class Transferreceiptnew_controller extends MX_Controller
{

    public function __construct()
    {
        parent::__construct();
        $this->load->model('document/transferreceiptnew/Transferreceiptnew_model');
    ...
    public function FSxCTWITransferReceiptDataTable()
    {
        ...
        $aResList   = $this->Transferreceiptnew_model->FSaMTRNList($aData);
        $aGenTable  = array(
            'aAlwEvent'         => $aAlwEvent,
            'aDataList'         => $aResList,
            'nPage'             => $nPage,
    ...
    public function FSvCTWITransferReceiptPageAdd()
    {
        try {
            ...
            $this->Transferreceiptnew_model->FSxMTWIClearPdtInTmp($tTblSelectData);
            ...
            $aDataUserGroup = $this->Transferreceiptnew_model->FSaMASTGetShpCodeForUsrLogin($aDataShp);
    ...
    public function FSvCTWITransferReceiptPageEdit()
    {
        try {
            ...
            $this->Transferreceiptnew_model->FSxMTWIClearPdtInTmp($aWhereClearTemp);
            ...
            $aDataDocHD = $this->Transferreceiptnew_model->FSaMTWIGetDataDocHD($aDataWhere);
            ...
            $aDataDocHDRef = $this->Transferreceiptnew_model->FSaMTWIGetDataDocHDRef($aDataWhere, $aDataWhereAddress);
            // Move Data DT TO DTTemp
            $this->Transferreceiptnew_model->FSxMTWIMoveDTToDTTemp($aDataWhere);
    ...
    public function FSoCTWIPdtAdvTblLoadData()
    {
        try {
            ...
            // $aDataDocDTTemp     = $this->Transferreceiptnew_model->FSaMTWIGetDocDTTempListPage($aDataWhere);
            // $aDataDocDTTempSum  = $this->Transferreceiptnew_model->FSaMTWISumDocDTTemp($aDataWhere);
    ...
    public function FSoCTWIAdvTblShowColList()
    {
        try {
            ...
            $aAvailableColumn      = $this->Transferreceiptnew_model->FSoMTWIlableColumn('TCNTPdtTwiDT');
    ...
    public function FSoCTWIAddPdtIntoDocDTTemp()
    {
        try {
            ...
            for ($nI = 0; $nI < count($aTWIPdtData); $nI++) {
                ...
                // $cTWIPrice    = $this->Transferreceiptnew_model->FSaMTWIGetPriceBYPDT($tTWIPdtCode);
                ...
                $nTWIMaxSeqNo = $this->Transferreceiptnew_model->FSaMTWIGetMaxSeqDocDTTemp($aDataWhere);
                ...
                $aDataPdtMaster     = $this->Transferreceiptnew_model->FSaMTWIGetDataPdt($aDataPdtParams);
                // นำรายการสินค้าเข้า DT Temp
                $nStaInsPdtToTmp    = $this->Transferreceiptnew_model->FSaMTWIInsertPDTToTemp($aDataPdtMaster, $aDataPdtParams);

                // ถ้าเลือกนำเข้าข้อมูล จะต้องไปทำให้สินค้าใน CN ว่าถูกใช้งานแล้ว
                if ($tTypeInsPDT == 'CN') {
                    // $this->Transferreceiptnew_model->FSaMTWIUpdatePDTInCN($tDocRefSO,$tSeqItemSO);
                }
            }
    ...
    public function FSoCTWIDeleteEventDoc()
    {
        try {
            ...
            $aResDelDoc = $this->Transferreceiptnew_model->FSnMTWIDelDocument($aDataMaster);
    ...
    public function FSvCTWIRemovePdtInDTTmp()
    {
        try {
            ...
            $this->Transferreceiptnew_model->FSnMTWIDelPdtInDTTmp($aDataWhere);
    ...
    public function FSvCTWIRemovePdtInDTTmpMulti()
    {
        try {
            ...
            $this->Transferreceiptnew_model->FSnMTWIDelMultiPdtInDTTmp($aDataWhere);
    ...
    public function FSoCTWIAddEventDoc()
    {
        try {
            ...
            $this->Transferreceiptnew_model->FSxMTWIAddUpdateHD($aDataMaster, $aDataWhere, $aTableAddUpdate);

            // Add Update Document HDRef
            $this->Transferreceiptnew_model->FSxMTWIAddUpdateHDRef($aDataPdtTwiHDRef, $aDataWhere);

            // Update Doc No Into Doc Temp
            $this->Transferreceiptnew_model->FSxMTWIAddUpdateDocNoToTemp($aDataWhere, $aTableAddUpdate);

            // Move Doc DTTemp To DT
            $this->Transferreceiptnew_model->FSaMTWIMoveDtTmpToDt($aDataWhere, $aTableAddUpdate);
    ...
    {
        $aCalDTTemp = $this->Transferreceiptnew_model->FSaMTWICalInDTTemp($paParams);
    ...
    public function FSoCTWIEventCancel()
    {
        ...
        $aStaApv    = $this->Transferreceiptnew_model->FSvMTWICancel($aDataUpdate);

        // อัพเดทใน CN ให้กลับไปใช้งานได้
        $this->Transferreceiptnew_model->FSvMCheckDocumentInCN('CANCEL', $aDataUpdate);
    ...
    public function FSoCTWIEditPdtIntoDocDTTemp()
    {
        try {
            ...
            $this->db->trans_begin();
            $this->Transferreceiptnew_model->FSaMTWIUpdateInlineDTTemp($aDataUpdateDT, $aDataWhere);
    ...
    public function FSoCTWISelectPDTInCN()
    {
        ...
        $aDataCN = $this->Transferreceiptnew_model->FSaMTWIGetPDTInCN($aWhere);
        $aDataViewCN = array(
            'aDataCN'       => $aDataCN
        );
    ...
    public function FSoCTWIApproved()
    {
        ...
        $aStaApv = $this->Transferreceiptnew_model->FSvMTWIApprove($aDataUpdate);
        try {
            $aMQParams = [
    ...
    public function FSaCTWIRefDoc()
    {
        ...
        $aPdtDataResult = $this->Transferreceiptnew_model->FSoMTWIEventGetPdtIntDTBch($aDataTWIGetPdtIntBch);
        $aDataWhere = array(
            'FTBchCode' => $tTWIBchCode,
            'FTXthDocNo' => $tTWIDocNo,
            'FTXthDocKey' => 'TCNTPdtTwiHD',
        ...
        $this->Transferreceiptnew_model->FSnMTWIDelPdtInDTTmpWhereSession($aDataWhereDelete);

        if (!empty($aPdtDataResult)) {
            foreach ($aPdtDataResult as $aData) {
                $cTBIPrice = $this->Transferreceiptnew_model->FSaMTWIGetPriceBYPDT($aData['FTPdtCode']);
                ...
                $nTBIMaxSeqNo = $this->Transferreceiptnew_model->FSaMTWIGetMaxSeqDocDTTemp($aDataWhere);
                ...
                if ($aDataPdtMaster['rtCode'] == 1) {
                    $this->Transferreceiptnew_model->FSoMTWIEventInsertPdtIntDTBchToTemp($aDataPdtMaster, $aDataPdtParams);
                }
            }
        }
    ...
    public function FSaCTWIGetWahRefDoc()
    {
        ...
        $aData = $this->Transferreceiptnew_model->FSaMTWIGetWahDocumentRef($aWhere);
