...
class Promotion_controller extends MX_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('document/promotion/Promotionstep4bchcondition_model');
    ...
    public function FSxCPromotionDataTable()
    {
        ...
        $aResList = $this->Promotion_model->FSaMHDList($aData);
        $aGenTable = array(
            'aAlwEvent' => $aAlwEvent,
            'aDataList' => $aResList,
            'nPage' => $nPage,
    ...
    public function FSxCPromotionAddPage()
    {
        ...
        $this->Promotion_model->FSxMClearInTmp($aClearInTmpParams);

        if ($tUserLevel != "HQ") { // In BCH or SHP
            $aGetPdtPmtHDBchInTmpPageAllParams = [
                'tUserSessionID' => $tUserSessionID,
            ];
            $bIsNoRecord = $this->Promotionstep4bchcondition_model->FSnMGetPdtPmtHDBchInTmpPageAll($aGetPdtPmtHDBchInTmpPageAllParams) <= 0;
            ...
            if ($bIsNoRecord) {
                ...
                $this->Promotionstep4bchcondition_model->FSaMPdtPmtHDBchToTemp($aPdtPmtHDBchToTempParams);
            }
        }
    ...
    public function FSaCPromotionAddEvent()
    {
        try {
            ...
            $this->Promotion_model->FSaMAddUpdateHD($aDataMaster);
            $this->Promotion_model->FSaMAddUpdateHDL($aDataMaster);
            $this->Promotion_model->FSaMAddUpdatePdtPmtHDCst($aDataMaster);

            $aUpdatePgtStaGetPdtInTmpFromHDParams = [
                'tStaGetPdt' => $aDataMaster['FTPmhStaGetPdt'],
                'tUserSessionID' => $tUserSessionID
            ];
            $this->Promotion_model->FSaMUpdatePgtStaGetPdtInTmpFromHD($aUpdatePgtStaGetPdtInTmpFromHDParams);

            $aUpdateDocNoInTmpParams = [
                'tDocNo' => $aDataMaster['FTPmhDocNo'],
                'tUserSessionID' => $tUserSessionID
            ];
            $this->Promotion_model->FSaMUpdateDocNoInTmp($aUpdateDocNoInTmpParams); // Update DocNo ในตาราง Doctemp
            ...
            $this->Promotion_model->FSaMTempToPdtPmtDT($aTempToTBParams); // คัดลอกข้อมูลจาก Temp to PdtPmtDT
            $this->Promotion_model->FSaMTempToPdtPmtCB($aTempToTBParams); // คัดลอกข้อมูลจาก Temp to PdtPmtCB
            $this->Promotion_model->FSaMTempToPdtPmtCG($aTempToTBParams); // คัดลอกข้อมูลจาก Temp to PdtPmtCG
            ...
            $this->Promotion_model->FSbUpdatePmhStaPdtExcInHD($aUpdatePmhStaPdtExcInHDParams);
            /*===== End Update FTPmhStaPdtExc in HD ====================================*/

            $aClearInTmpParams = [
                'tUserSessionID' => $tUserSessionID,
            ];
            $this->Promotion_model->FSxMClearInTmp($aClearInTmpParams); // ล้าง Temp
    ...
    public function FSvCPromotionEditPage()
    {
        ...
        $this->Promotion_model->FSaMPdtPmtHDBchToTemp($aDataToTempParams);
        ...
        $this->Promotion_model->FSaMPdtPmtDTToTemp($aDataToTempParams);
        $this->Promotion_model->FSaMPdtPmtCBToTemp($aDataToTempParams);
        ...
        $aResult = $this->Promotion_model->FSaMGetHD($aGetHDParams); // Data TCNTPdtPmtHD

        $aGetPdtPmtHDCstParams = array(
            'tDocNo' => $tDocNo,
            'nLngID' => $nLangEdit
        );
        $aPdtPmtHDCstResult = $this->Promotion_model->FSaMGetPdtPmtHDCst($aGetPdtPmtHDCstParams); // Data TCNTPdtPmtHDCst
        /*===== End Get Data ===========================================================*/
    ...
    public function FSaCPromotionEditEvent()
    {
        try {
            ...
            $aHDData = $this->Promotion_model->FSaMGetHD($aGetHDParams);
            ...
            if ($bIsStaApv) { // อนุมัติแล้ว แต่ต้องการ เปิด-ปิด โปรโมชั่น
                $this->Promotion_model->FSbMAddUpdatePmhStaClosedHD($aDataMaster);
            } else { // ขั้นตอนปกติ
                $this->Promotion_model->FSaMAddUpdateHD($aDataMaster);
                $this->Promotion_model->FSaMAddUpdateHDL($aDataMaster);
                $this->Promotion_model->FSaMAddUpdatePdtPmtHDCst($aDataMaster);
    ...
    public function FStCPromotionUniqueValidate()
    {
        ...
        if ($this->input->is_ajax_request()) { // Request check
            ...
            $bIsDocNoDup = $this->Promotion_model->FSbMCheckDuplicate($tPromotionDocCode);
    ...
    public function FStCPromotionDocCancel()
    {
        ...
        $aStaCancel = $this->Promotion_model->FSaMDocCancel($aDocCancelParams);
    ...
    public function FStCPromotionDocApprove()
    {
        ...
        $aStaApv = $this->Promotion_model->FSaMDocApprove($aDocApproveParams);
    ...
    public function FStPromotionDeleteDoc()
    {
        ...
        $this->Promotion_model->FSaMDelMaster($aDelMasterParams);
