        ...
        foreach($paPdtDataLang AS $aPdt){
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                if($aPdt['FTPdtName']){
                    ...
                    $tSql = "INSERT INTO TCNMPdt_L (FTPdtCode, FNLngID, FTPdtName, FTPdtNameOth, FTPdtNameABB, FTPdtRmk)
                             VALUES ('".$paPdtWhere["FTPdtCode"]."', ".$aPdt['FNLngID'].", N'".$aPdt["FTPdtName"]."', N'".$aPdt["FTPdtNameOth"]."', N'".$aPdt["FTPdtNameABB"]."', N'".$aPdt["FTPdtRmk"]."')";
                    $this->db->query($tSql);
                }else{
                    $aPdtDelete = array(
                        'FTPdtCode' => $paPdtWhere['FTPdtCode'],
    ...
    */
    ...
    public function FSaMPDTCtyCheckByID($pUsrCode){
        $tSQL = "SELECT 
                ...
                FROM [TCNTUsrGroup] USRG WITH (NOLOCK)
                LEFT JOIN [TCNMAgency] AGN WITH (NOLOCK) ON USRG.FTAgnCode = AGN.FTAgnCode
                LEFT JOIN [TCNMCountry_L] CUNL WITH (NOLOCK) ON CUNL.FTCtyCode = AGN.FTCtyCode
                WHERE USRG.FTUsrCode = '$pUsrCode'";
        $oQuery = $this->db->query($tSQL);
        return $oQuery->row_array();
    }
