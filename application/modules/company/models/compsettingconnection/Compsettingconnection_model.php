...
class Compsettingconnection_model extends CI_Model {
    ...
    //Return : Status Add Update Master
    //Return Type : Array
    public function FSaMCompSetConAddUpdateMasterUrlMQ($paDataUrlObj,$paDataMQMain,$paDataMQDoc,$paDataMQSale,$paDataMQReport){
        try{
            if($this->db->affected_rows() > 0){
                $aStatus = array(
                    'rtCode' => '1',
    ...
    public function FSaMCompSetConAddUpdateMasterUrlAuthorUpdate($paDataUrlObj,$paDataUrlObjlogin,$paOldKeyUrl){
        ...
        try{
            ...
            if($tUrlType == '2' || $tUrlType == '14' || $tUrlType == '15'){
                $this->db->set('FNUrlType'    , $paDataUrlObj['FNUrlType']);
                $this->db->set('FTUrlTable'   , $paDataUrlObj['FTUrlTable']);
                $this->db->set('FTUrlKey'     , $paDataUrlObj['FTUrlKey']);
                $this->db->set('FTUrlAddress' , $paDataUrlObj['FTUrlAddress']);
    ...
    //Return : data
    //Return Type : Array
    public function FSaMCompSetConAddUpdateMasterUrlMQUpdate($paDataUrlObj,$paDataMQMain,$paDataMQDoc,$paDataMQSale,$paDataMQReport,$paOldKeyUrl){
        try{
