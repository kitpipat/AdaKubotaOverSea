...
class Branch_controller extends MX_Controller {
    ...
    public function FSvCBCHDataList(){
		...
		$aResList = $this->Branch_model->FSnMBCHList($aData);
        $aGenTable  = array(
	...
	public function FSvCBCHGenViewAddress($paResList = '',$nCnfAddVersion = ''){
		...
		if(isset($paResList['roItem']['rtBchCode'])){
			...
			$aCnfAddEdit    = $this->Branch_model->FSvMBCHGetAddress($aData);
	...
	public function FSaCBCHAddEvent(){
		...
		$aResAdd		= $this->Branch_model->FSaMBCHAdd($aDataMaster);

		//เพิ่มคลัง 00001 00002 00003 16/03/2020 saharat
		$this->Branch_model->FSaMBCHWahDefaultInsert($aDataMaster);
		$this->Branch_model->FSaMBCHWahDefaultInsert_L($aDataMaster);
		...
		if(!empty($aDataMaster['FTAgnCode']) && $this->session->userdata('nSesUsrBchCount')!=0){
			...
			// $this->Branch_model->FSxMBCHUpdateUsrGrp($aDataMaster);
			$aDataUsrGroup = $this->Login_model->FSaMLOGGetDataUserLoginGroup($tSesUserCode);
	...
	public function FSaCBCHEditEvent(){
		...
		$aResAdd = $this->Branch_model->FSaMBCHUpdate($aDataMaster);
		if($aResAdd['rtCode'] == '1'){
			if($tBranchImage != $tBranchImageOld){
				$aImageUplode = array(
					'tModuleName'       => 'company',
	...
	public function FSvCBCHEditPage($ptBchCode = '',$ptUserLevel = ''){
		...
		$aResList       = $this->Branch_model->FSaMBchSearchByID($aData);
	...
	public function FSaCBCHDeleteEvent(){
		...
		$aResDel   = $this->Branch_model->FSnMBCHDel($aDataMaster);
		$aDeleteImage = array(
			'tModuleName'  => 'branch',
			'tImgFolder'   => 'branch',
			'tImgRefID'    => $tIDCode,
	...
	public function FSaCBCHGetDataImport(){
		...
		$aGetData 					= $this->Branch_model->FSaMBCHGetTempData($aDataSearch);
	...
	public function FSaCBCHImportDelete(){
		...
		$aResDel   = $this->Branch_model->FSaMBCHImportDelete($aDataMaster);
	...
	public function FSaCBCHImportMove2Master(){
		...
		$aResult = 	$this->Branch_model->FSaMBCHImportMove2Master($aDataMaster);
					$this->Branch_model->FSaMBCHImportMove2MasterAndInsWah($aDataMaster);
					$this->Branch_model->FSaMBCHImportMove2MasterAndReplaceOrInsert($aDataMaster);
					$this->Branch_model->FSaMBCHImportMove2MasterDeleteTemp($aDataMaster);
	...
	public function FSaCBCHImportGetItemAll(){
		$aResult  = $this->Branch_model->FSaMBCHGetTempDataAtAll();
